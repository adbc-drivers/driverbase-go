...
func formatCSource(in []byte) ([]byte, error) {
	r := bytes.NewReader(in)
	cmd := exec.Command("clang-format", "-style={BasedOnStyle: Google, ColumnLimit: 0, DerivePointerAlignment: false}")
	cmd.Stdin = r
	out, err := cmd.Output()
	if err != nil {
		var ee *exec.ExitError
...
func main() {
	...
	outDir := path.Join(*driverPkg, "pkg")
	...
	process(tmplData{
		...
		DriverImport:  pkg[0].PkgPath,
		DriverPackage: pkg[0].Name,
		Prefix:        *prefix,
...
func process(data any, specs []pathSpec) {
